...
export default function AddEventForm({ onCreate, onCancel }) {
  ...
  return (
    <Card as="form" onSubmit={handleSubmit}>
      <h3 style={{marginTop:0}}>เพิ่มอีเวนต์ใหม่</h3>

      <Row>
        <div>
          <Label>ชื่ออีเวนต์</Label>
          <Input value={form.name} onChange={(e)=>setField("name", e.target.value)} placeholder="เช่น QRRA: The Crowning Step" />
        </div>
        <div>
          <Label>หมวดหมู่ (Category)</Label>
          <Input value={form.Category} onChange={(e)=>setField("Category", e.target.value)} placeholder="เช่น Concert/Musical" />
        </div>
      </Row>
      ...
      <Row>
        <div>
          <Label>วันที่</Label>
          <Input value={form.date} onChange={(e)=>setField("date", e.target.value)} placeholder="เช่น 14 ธ.ค. 2025" />
        </div>
        <div>
          <Label>เวลา</Label>
          <Input value={form.time} onChange={(e)=>setField("time", e.target.value)} placeholder="เช่น 13:00" />
        </div>
      </Row>
      ...
      <Row>
        <div>
          <Label>สถานที่</Label>
          <Input value={form.location} onChange={(e)=>setField("location", e.target.value)} placeholder="เช่น Centerpoint Entertainment" />
        </div>
        <div>
          <Label>รูปภาพหลัก (image)</Label>
          <Input value={form.image} onChange={(e)=>setField("image", e.target.value)} placeholder="เช่น TGS.jpg" />
        </div>
      </Row>
      ...
      <Row>
        <div>
          ...
          <Input value={form.venueImage} onChange={(e)=>setField("venueImage", e.target.value)} placeholder="เช่น TGS_venue.jpg" />
        </div>
        <div>
          <Label>รูปแบนเนอร์ (Banner)</Label>
          <Input value={form.Banner} onChange={(e)=>setField("Banner", e.target.value)} placeholder="เช่น TGS_Banner.jpg" />
        </div>
      </Row>
      ...
      <div>
        ...
        <Textarea value={desc.main} onChange={(e)=>setDesc("main", e.target.value)} placeholder="รายละเอียดหลักของงาน" />
      </div>
      <Row>
        <div>
          <Label>สรุปราคา (description.ticket)</Label>
          <Input value={desc.ticket} onChange={(e)=>setDesc("ticket", e.target.value)} placeholder="เช่น VIP Album 5,200 / VIP 4,500 / A 2,199 / B 1,299" />
        </div>
        <div>
          <Label>เงื่อนไข (description.condition)</Label>
          <Input value={desc.condition} onChange={(e)=>setDesc("condition", e.target.value)} placeholder="เช่น ไม่สามารถยกเลิกหรือเปลี่ยนแปลงบัตรได้ ..." />
        </div>
      </Row>
